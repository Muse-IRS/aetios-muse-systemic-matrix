...
The Silence-First Framework is evaluated strictly as a
\emph{pre-inference causal regulator}.
It is not assessed on output quality, semantic correctness,
ethical alignment, or task performance.

All measurements concern system-level behavior:
stability, load regulation, and inference gating.

\subsection{Simulation Assumptions}

The experimental environment assumes:
\begin{itemize}
\item deterministic inference cost per call,
\item bounded computational resources,
\item bursty and adversarial input patterns,
\item stochastic background noise.
\end{itemize}

These assumptions approximate real-world large-scale
AI deployment conditions.

\subsection{Phase Construction}

The causal phase increment $\Delta \Phi$ is modeled as a
composite quantity aggregating:
\begin{itemize}
\item informational density (proxy entropy),
\item temporal drift,
\item thermodynamic dissipation,
\item stochastic jitter.
\end{itemize}

No single component dominates $\Phi$ by construction.

\subsection{Threshold Selection}

The divergence threshold $\varepsilon$ is selected empirically
to balance responsiveness and stability.
It is not optimized for maximal throughput.

\subsection{Reproducibility}

All experiments are reproducible using synthetic inputs
and deterministic random seeds.
Public reference implementations are provided separately.

\subsection{Interpretation Boundaries}

SSF metrics should not be interpreted as:
\begin{itemize}
...
They strictly quantify causal load and gating behavior.
